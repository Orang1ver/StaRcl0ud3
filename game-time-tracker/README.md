...
双击 **`设置开机自启.cmd`** 一次做三件事（换电脑、挪过文件夹之后也可以再跑一遍）：

1. 写注册表「登录后自动启动」；
2. 建计划任务 `GameTimeTrackerWatchdog`：**每 5 分钟检查一次**，发现没在跑就拉起来。
   已经在跑的时候，新进程会立刻退出，不会有第二个实例（也不会重复计时）；
3. 立刻启动一次，并打印当前状态。

为什么要第二条：只靠注册表启动，遇到安全软件拦截、被误杀、启动被延迟时，
它可能一直没跑起来 —— 那段时间的游玩就丢了（2026-09-12 就发生过一次：原神 11:15-11:46 全程没记上）。
有了看门任务，最多 5 分钟就会补上。

**排查用**：文件夹里的 `tracker.log` 会记录每次启动成功 / 重复启动的时间（超过 200KB 自动只留最后 200 行）。
怀疑「今天没记上」时先看它：如果是「启动成功」出现在你玩游戏之后，那就是当时记录器还没跑起来。
